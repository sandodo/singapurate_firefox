...
SingapuRatePanel.port.on("goUrl", function(msg) {
	...
	var sParseMsg = msg;
	var resArray = sParseMsg.split("?");
	for(var i=0; i < resArray.length; i++) 
	{
		var strTemp = resArray[i];
		if ( strTemp.indexOf("wrs_url=") >= 0 )
		{
			var resArrayWrsUrl = strTemp.split("&");
			for(var j=0; j < resArrayWrsUrl.length; j++) 
			{
				var strTempWrsUrl = resArrayWrsUrl[j];
				if ( strTempWrsUrl.indexOf("wrs_url=") >= 0 )
				{
					//found domain name
					var resArrayDomain = strTempWrsUrl.split("=");
					var sOnlyDomainName = SingapuRateUtils.SR_Utilities.getOnlyDomainName(resArrayDomain[1], SingapuRateUtils.SR_Utilities.SingapuRateDomainCheckDepth);
					if(sOnlyDomainName != "")
					{
						var retResults = SingapuRateUtils.SR_WebsiteRatings.isBlockedInCache(sOnlyDomainName);
						var strKeyCurDomainUrlIdx 	= SingapuRateUtils.SR_Utilities.SingapuRatePrefKeyCurDomainUrlIdx;
						var strKeyCLEntryDatesWI    = SingapuRateUtils.SR_Utilities.SingapuRatePrefKeyCLEntryDates + retResults[strKeyCurDomainUrlIdx];
						...
						SingapuRateSS.storage[strKeyCLEntryDatesWI] = "" + iIntegerFiveDaysAgo;
	    ...
	    try
	    {
			...
			else
			{
				...
				if( retResults[SingapuRateUtils.SR_Utilities.SingapuRateParamNameCategoryId] > 0 )
				{
					...
					var strKeyCLLastActionTimeWI = SingapuRateUtils.SR_Utilities.SingapuRatePrefKeyCLLastActionTime + retResults[strKeyCurDomainUrlIdx];
					var strKeyCLIdxToDomainsWI   = SingapuRateUtils.SR_Utilities.SingapuRatePrefKeyCLIdxToDomains + retResults[strKeyCurDomainUrlIdx];
					//need to query soap service to update the website information
					if( retResults[SingapuRateUtils.SR_Utilities.SingapuRateParamNameNeedToQueryAgain] === true 
					                  && (SingapuRateSS.storage[strKeyCLIdxToDomainsWI] != sOnlyDomainName 
					                                || typeof SingapuRateSS.storage[strKeyCLLastActionTimeWI] === 'undefined'
					                                || parseInt(SingapuRateSS.storage[strKeyCLLastActionTimeWI]) < (Date.now() / 1000 - 30)) )
					{
			...
			if(retResults[SingapuRateUtils.SR_Utilities.SingapuRateParamNameNeedToQueryAgain] === true)
			{
				...
				var strKeyCLIdxToDomainsWI   = SingapuRateUtils.SR_Utilities.SingapuRatePrefKeyCLIdxToDomains + retResults[strKeyCurDomainUrlIdx];
				//need to query soap service to update the website information
				if( SingapuRateSS.storage[strKeyCLIdxToDomainsWI] != sOnlyDomainName 
							|| typeof SingapuRateSS.storage[strKeyCLLastActionTimeWI] === 'undefined'
							|| parseInt(SingapuRateSS.storage[strKeyCLLastActionTimeWI]) < (Date.now() / 1000 - 60) )
				{
					//As it is more than 1 minutes interval, or url domain name changed, query soap service for now.
					SingapuRateSS.storage[strKeyCLLastActionTimeWI] = Date.now() / 1000;
					SingapuRateSS.storage[strKeyCLIdxToDomainsWI] = sOnlyDomainName;
